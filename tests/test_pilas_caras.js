...
// Corchete 103B φ16 nivel 2, cara sup, 2 capas anidadas (gap 0 → δ = φ = 1.6):
...
//   prof(ext,2) = 4 + 0 = 4       → B auto = 600 − 2·4 = 592
//   capa 2 (anidada, δ=1.6): B − 2δ = 588.8; patas A/C − δ = 28.4;
//     el tramo baja δ → y = 24.4 − 1.6 = 22.8; las PUNTAS siguen alineadas
//     (24.4 − 30 = −5.6 y 22.8 − 28.4 = −5.6).
...
ok(close(co[1].dims.B, 588.8) && close(co[1].dims.A, 28.4) && close(co[1].dims.C, 28.4),
  'capa 2 anidada: B − 2δ = 588.8 y patas − δ = 28.4 (δ = φ = 1.6)');
ok(close(co[1].puntos[1].y, 22.8), 'capa 2 más adentro: 24.4 − 1.6 = 22.8 (=' + co[1].puntos[1].y + ')');
ok(close(co[0].puntos[0].y, -5.6) && close(co[1].puntos[0].y, -5.6),
  'las PUNTAS de las patas quedan alineadas entre capas (−5.6)');
...
//     → B auto = largo_local − 2·3.8 = ancho − 7.6 = 30 − 7.6 = 22.4
...
//   CONTROL geométrico independiente de la fórmula: la punta del corchete tiene
//   que quedar EXACTAMENTE en la cara interior de la pata lateral del estribo,
//   cuyo eje está en z = ±(15 − 3 − 0.4) = ±11.6 → cara interior ±11.2 = B/2.
//   prof_ef(sup,2) = 4 + 0.8 = 4.8 → eje del tramo = 24.4: SIGUE TANGENTE al
//     estribo (1.2 de separación) en su nuevo plano.
//   capa 2: B − 2δ = 22.4 − 3.2 = 19.2; patas 28.4; y = 22.8.
// El tramo ahora corre en Z (de −11.2 a +11.2), no en X.
console.log('\nA2 — el MISMO corchete VOLTEADO (marco, pilas y recubs permutados):');
const oAv = run([estribo(), corchete(true)]);
const cov = plsDe(oAv, 'CO');
ok(close(cov[0].dims.B, 22.4),
  'volteado capa 1: B auto = 30 − 2·(recub_lat + φest) = 22.4 — pila Y recub PERMUTADOS (=' + cov[0].dims.B + ')');
...
ok(close(maxE(cov[0], 'z'), 11.2) && close(minE(cov[0], 'z'), -11.2),
  'volteado: el tramo corre en Z de −11.2 a +11.2 (B/2 = 11.2)');
ok(close(maxE(cov[0], 'z'), maxE(unaDe(oAv, 'ES'), 'z') - PHI_ES / 2),
  'volteado: la punta cae en la CARA INTERIOR de la pata lateral del estribo (tangente, sin hueco)');
ok(close(maxE(cov[0], 'x'), 0) && close(minE(cov[0], 'x'), 0), 'volteado: ya no corre en X');
ok(close(cov[1].dims.B, 19.2) && close(cov[1].dims.A, 28.4),
  'volteado capa 2 anidada: B − 2δ = 22.4 − 3.2 = 19.2, patas 28.4');
ok(close(cov[1].puntos[1].y, 22.8) && close(cov[1].puntos[0].y, -5.6),
  'volteado capa 2: y = 22.8 y las puntas siguen alineadas (−5.6)');
