...
- [ ] PC.15 Informe PDF de reclamo + envío por correo
	- [x] PC.15.1 Agregar `xhtml2pdf` a requirements.txt
	- [x] PC.15.2 Crear template Jinja2 `templates/pdf/reclamo_informe.html` — layout del informe: header (logo, correlativo, fecha, estado, prioridad), sección Registro (título, descripción, proyecto/cliente, categoría, detectado por, fecha detección), sección Análisis (respuesta, causa Ishikawa, área responsable, kilos, tiempo respuesta), sección Acciones correctivas/preventivas (tabla), sección Validación (resultado, observaciones, fecha, validado por), imágenes antecedentes + respuesta (thumbnails inline ~300px), timeline de seguimientos
	- [x] PC.15.3 CSS embebido para PDF — tipografía, márgenes, tablas, imágenes, saltos de página, colores por estado/prioridad
	- [x] PC.15.4 Backend: endpoint `GET /reclamos/{id}/pdf` — carga detalle completo + imágenes (base64 inline), renderiza template, devuelve `Response(application/pdf)`. Permisos: admin, admin2, cubicador (si es propio), usc
	- [x] PC.15.5 Frontend: botón "📄 PDF" en modal de detalle del reclamo — fetch+blob abre en nueva pestaña. Visible para roles con permiso. Botón "📧 Enviar" como placeholder gris (próximamente)
...
- [x] PC.17 Split `features/reclamos/index.js` (3,067 líneas → 8 archivos) — extraer dominios a archivos independientes, eliminar clases legacy muertas, actualizar bootstrap.js
	- [x] PC.17.1 Extraer `constants.js` — dicts de colores/labels, estado global (`_reclamoActual`, `_FECHA_OPERATIVA`, etc.)
	- [x] PC.17.2 Extraer `helpers.js` — normalizers, adapters legacy, `_calcDiasReclamo`, `_diasBadgeHtml`, `_formatCorrelativoCalidad`
	- [x] PC.17.3 Extraer `dashboards.js` — `switchRecTab`, `loadRecLanding`, `loadRecAdminDashboards` (9 charts)
	- [x] PC.17.4 Extraer `presentaciones.js` — `loadPresentaciones`, render, nav, `guardarPresentacion`, `loadPresStats`
	- [x] PC.17.5 Extraer `list.js` — `loadReclamos`, filtros, scope toggle, `loadRecUsersDropdown`
	- [x] PC.17.6 Extraer `form.js` — modal helpers (PC.8), `crearReclamo`, crear proyecto/calc/const inline
	- [x] PC.17.7 Extraer `detail.js` — detalle completo: render, permisos, edición, estado, análisis, validación, acciones CRUD, imágenes/drop zones, seguimientos, ishikawa
	- [x] PC.17.8 Limpiar `index.js` — dejar solo IIFE registration + `_loadReclamosModule` + exports
	- [x] PC.17.9 Actualizar `bootstrap.js` — cargar 8 scripts en orden correcto de dependencias
	- [x] PC.17.10 Eliminar clases muertas — `FormRenderer`, `ReclamoPresenter`, `ReclamoUtils` eliminadas (~250 líneas). `ImageRenderer` mantenida (activa en presentaciones.js)
	- [x] PC.17.11 Smoke test: listado + filtros + scope toggle
