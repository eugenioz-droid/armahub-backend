...
| 5L.11.10 | Poblar constructoras + vincular obras existentes (el valor real de la caluga). Modal "🔗 Vincular" en cada fila: muestra ya-vinculadas (con desvincular) + obras SIN constructora con checkboxes y búsqueda (prefill del prefijo). Endpoints nuevos: `GET /proyectos-sin-constructora`, `POST /constructoras/{id}/vincular-obras` (masivo). `asignar-constructora` abierto a gestión de clientes (usc/admin). Método MANUAL | ☑ | YO |
| 5L.11.11 | Renombrar caluga a "Constructoras / Clientes" (registry title, tabLabels, botón, títulos). Alinear con glosario | ☑ | YO |
| 5L.11.12 | Campo `tipo` (constructora/retail/otro) — migración 80. Mismo modelo (empresa→hijos vía constructora_id), rótulo adaptativo del hijo: constructora→Obra, retail→Sede, otro→Proyecto. Soporta clientes retail (Sodimac→sedes) sin refactor. Selector de tipo en el modal + columna Tipo en la tabla | ☑ | YO |
