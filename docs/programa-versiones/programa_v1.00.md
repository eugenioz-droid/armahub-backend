...
| 5L.11.14 | **Empresa como ENTIDAD propia (decisión del usuario) + 2 sub-tabs + columna Reclamos.** Caluga "Clientes" (nombre global, engloba obras/tiendas) con 2 sub-tabs: **Obras / Tiendas** (primario) — lista obras, edita clasificación + EMPRESA (selector de entidad, no texto) + calculista + fecha; columna nueva **Reclamos** junto a Kilos (`GET /proyectos` trae `n_reclamos` por obra). **Empresas** (secundario) — CRUD de empresas reactivando tabla `constructoras` (nombre, tipo constructora/retail/otro, RUT, contacto, email...), con conteo de obras y borrado bloqueado si tiene obras. La obra referencia la empresa por `constructora_id`. `proyectos.empresa` (texto, mig 81) queda inerte | ☑ | YO |
