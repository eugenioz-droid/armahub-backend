...
| 5M.8.8 | **Integración editor ↔ catálogo (PENDIENTE — lo señaló Eugenio).** Hoy al guardar una figura dibujada, el `POST` PISA `angulos`/`parciales`/`radio` del catálogo con lo derivado del dibujo. Pero una cosa es la geometría dibujada y otra los datos que el catálogo ya tenía (de la herramienta de detallado). Falta una integración controlada: ver qué tiene el catálogo vs. qué se dibujó, y decidir (no pisar a ciegas). La idea es **editar el catálogo DESDE el editor de figuras** de forma consciente. Fase propia, requiere discovery. Por ahora: dibujar pisa (asumido en el levantamiento inicial). | ☐ | TÚ+YO |
