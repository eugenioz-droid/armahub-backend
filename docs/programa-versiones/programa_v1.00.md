...
| 5M.8.7 | **Diseñador — modo CURVAS (2D) + etiquetas manuales (PLANIFICADO, fase propia, PRIMERO).** Modo "Curva": tramo tipo `arco` con radio controlable (slider/punto de control) → `<path>` SVG con comando `A` (nativo, sin librería). Parámetros de curva: radio, altura de cuerda, desarrollo — se ingresan 2, el sistema calcula el 3º (longitud de arco). **Etiquetas manuales** `{texto, x, y, tipo:medida/letra/ángulo}` que el usuario coloca/arrastra (auto-posición no basta en curvas). Modelo: `tramos` acepta `{tipo:"recto"}` o `{tipo:"arco",...}`. Riesgo bajo (aditivo; el modo recto no cambia). **PRINCIPIO:** diseñar el modelo de tramos COMPLETO ahora (recto/arco + plano de doblez) para que la curva-3D futura no requiera rehacer. | ☐ | TÚ+YO |
| 5M.8.5 | **Diseñador 3D (toggle 2D/3D) — PLANIFICADO (fase más grande, DESPUÉS de curvas).** Toggle 2D/3D. En 3D: visor Three.js `TubeGeometry` (barra=tubo de radio Ø siguiendo la polilínea), cargado ON-DEMAND (solo al activar 3D → no pesa el resto). **Entrada en 3D por PARÁMETROS de tramo, NO clicks libres** (largo + giro + plano de doblez XY/XZ/YZ) — clicks en pantalla plana son ambiguos en profundidad; parámetros = preciso y sin ambigüedad; el visor 3D se rota para revisar. Modelo `dim:"3D"` + plano por tramo (ya preparado). Las 2D no se tocan. Curva-3D = arco con orientación 3D (mismo concepto de tramo → no rehacer). Riesgo contenido (Three.js solo en 3D); el costo es esfuerzo, no romper. | ☐ | TÚ+YO |
