...
| 5M.9 | **Vista PLANA al filtrar por figura/tipología (edición eficiente).** Cuando hay filtro de nivel-barra activo (figura o tipología; lógica ampliable a diámetro), el Bar Manager muestra una TABLA ÚNICA de todas las barras del filtro (sin desplegables de agrupación), editable, PAGINADA (control de rendimiento). Columnas + ubicación (piso/sector/ciclo/eje) ya que no hay desplegable padre. Sin esos filtros, vista agrupada normal (intacta). Render de fila compartido (`_bmFilaBarraHTML`). **Optimistic update:** el PATCH devuelve la barra actualizada (peso/largo recalc); el front muta memoria + re-render local sin re-pedir la lista. FIX incluido: get_figura reventaba con 500 sobre cursor dict_row (impedía guardar). Diámetro: en la lógica, NO implementado (se ordena en la próxima iteración de filtros) | ☑ | YO |
