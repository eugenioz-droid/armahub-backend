...
| 5M.8.4 | **Diseñador — requisitos finos (Eugenio 23-jul), planificados.** (a) **Convención ángulo aSa Studio:** un doblez de 90° se guarda/exporta como **0** (así lo lee aSa); Bar Manager y exportación ya lo dejan en 0. El diseñador debe TRADUCIR a esta convención al guardar. PENDIENTE aclarar: ¿solo 90→0 o hay más mapeos? (b) **Máximo 4 ángulos (α1-α4):** el modelo de barra soporta 4. Si se dibuja una figura con más dobleces, el diseñador debe AVISAR y no permitir guardar algo que no cabe (poco recurrentes, no urgente). (c) **Letras de lado renombrables libres:** figuras con curva/gancho 180° usan parámetros distintos que el software de fabricación lee en columnas específicas → el usuario debe poder renombrar cada letra para que la medida quede en la columna correcta (ya hay input por lado; asegurar que el nombre libre se respete dibujo→guardado→Bar Manager→exportación). (d) **Nombrar ángulos como las tablas** + IDEAL verlos en el render con su nombre. (e) **FIX click REAL:** el SVG se recreaba en cada mousemove → el click caía sobre nodo destruido; ahora SVG persistente + capa dinámica (commit 40bb3d4). | 🔄 | TÚ+YO |
| 5M.8.5 | **Diseñador 3D (toggle 2D/3D) — planificado.** Idea de Eugenio: un toggle que cambia el lienzo a 3D. En 3D, vista con Three.js `TubeGeometry` (barra=tubo de radio Ø siguiendo la polilínea); el modelo de geometría suma un eje Z por tramo. Three.js se carga ON-DEMAND (solo al activar 3D) para no pesar el resto. Fase propia, más grande. Las 3D son las menos pero deben poder crearse acá. | ☐ | TÚ+YO |
