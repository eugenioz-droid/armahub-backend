...
| 5M.11.1 | **Paso 1 — Selección múltiple (sin tocar datos).** En la vista plana + modo edición: checkbox por fila + "seleccionar todas" (las visibles de la página), estado `_seleccion` (Set de ids), barra verde con contador ("N barras seleccionadas") + "Quitar selección". La selección se limpia al: cambiar de obra, salir de edición, re-buscar/cambiar filtro. La selección persiste entre páginas (es por id). Sin acción todavía — el panel de transformación + preview + aplicar llega en el Paso 2. | ☑ | YO |
