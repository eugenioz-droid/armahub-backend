...
| 5N.32 | **Acciones masivas — 2º nivel de duplicado (2026-07-31).** Barra de acciones masivas reorganizada en dos niveles. Nivel 2: **duplicar +piso** (clona las marcadas y las sube un piso), **duplicar -piso** (las baja), **duplicar seleccionados** (mismo piso). `ac2DuplicarPiso(dir)` mueve el piso una posición en `_ac2Pisos` (índice 0 = más bajo). Warning si la obra no tiene pisos configurados o si las barras ya están en el tope/piso más bajo. | ☑ | YO |
