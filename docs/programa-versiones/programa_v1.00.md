...
| 5M.3 | **Fase 3 — Edición de campos simples + toggle + auditoría (habilita edición YA).** `PATCH /barras/{id}` (permiso dueño de obra + admin; 403 claro si no). Campos editables: diam/largo_total/cant/cant_total/mult; recalcular peso en backend. **Tipología (marca) NO editable por ahora** (decisión usuario 2026-07: está ligada a figura/estructura, cambiarla a mano desalinea). Columnas `editado_por`/`editado_fecha`. Toggle candado 🔒 + warning + guardar-al-cerrar + celdas resaltadas. Auditoría `editar_barra` + panel de ediciones recientes (obra actual) al final del Bar Manager | ☐ | YO |
