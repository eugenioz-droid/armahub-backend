...
| 5L.11.13 | **REWORK del modelo (decisión del usuario):** la caluga gira en torno a la OBRA, no a una tabla de constructoras. La empresa y la clasificación son ATRIBUTOS de la obra. Migración 81: `proyectos.clasificacion` (obra/tienda/otro) + `proyectos.empresa` (texto libre). La caluga lista las obras existentes y edita su data (clasificación, empresa con autocompletado, calculista, descripción, fecha inicio). Filtros por clasificación/empresa/sin-empresa. Endpoints: `GET /proyectos` extendido + `GET /proyectos/empresas` (autocompletado) + `PATCH /proyectos/{id}` extendido. Objetivo: entrar y completar la data de las obras que YA existen | ☑ | YO |
