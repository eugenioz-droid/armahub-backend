...
| 5M.8.7 | **Diseñador — modo CURVAS + etiquetas manuales (planificado, fase propia).** Pedido Eugenio: para figuras curvas (gancho, cuerda), un modo manual donde se dibuja un arco controlando parámetros (radio, altura de cuerda; el desarrollo se calcula). Además **etiquetas manuales** que el usuario coloca/arrastra: de MEDIDA, de LETRA y de ÁNGULO. Es un salto de complejidad (modelo de etiquetas con posición + primitiva de arco SVG). Se hace DESPUÉS de estabilizar el dibujo por segmentos. También: acomodar/arrastrar las etiquetas auto (5M.8.4-f). | ☐ | TÚ+YO |
