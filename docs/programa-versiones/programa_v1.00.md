...
| FIX 5M.11 | **Filtro Eje/Losa NO aplica al elegir + reaparece sugerencia de contraseña de Chrome.** **CAUSA RAÍZ (auditada 2026-07-30, 6 subagentes):** eran DOS bugs distintos, ninguno del tipo de campo. (1) *No filtra:* la vista agrupada usa `GET /barras/elementos`, cuyo endpoint **no acepta el parámetro `eje`** (`barras.py:225`) → FastAPI descarta el `eje` en silencio y nunca aplica `AND eje=%s`. (2) *Pide clave:* hay un `<input type="password">` real de crear-usuario (`admin.html:37`) en el MISMO DOM de `/ui` (app de una sola página) → Chrome marca TODOS los text inputs como credenciales; el blindaje por-campo no puede ganarle. **Solución adoptada:** eje = `<select>` normal como sus hermanos (sin combobox/datalist); + FIX 5M.12 (backend eje) + FIX 5M.13 (password fuera del DOM). Se descartó el input-texto para el eje: para filtrar de una lista conocida, `<select>` es lo correcto. | ☑ | YO |
| FIX 5M.12 | **Backend: `GET /barras/elementos` acepta y aplica el filtro `eje`** (`barras.py:225`, agregar `eje` a la firma + `AND eje=%s`), para que el filtro de Eje de la vista agrupada filtre de verdad. Evaluar normalizar espacios (bug latente: `eje` se guarda crudo del CSV, variantes `24(A-E)` vs `24 (A-E)` rompen el match exacto). | ☐ | YO |
| FIX 5M.13 | **Quitar el `<input type="password">` persistente del DOM de `/ui`** (`admin.html:37`, crear-usuario): dejarlo `type="text"` y promoverlo a `password` por JS solo mientras el form "Nuevo usuario" está visible. Sin password en reposo, Chrome deja de tratar TODOS los filtros de texto como credenciales (arregla el "pide clave" de forma transversal, no campo por campo). | ☐ | YO |
| 5M.14 | **Refactor Bar Manager (anti-fragilidad) — PLANIFICADO, por etapas.** Diagnóstico (auditoría 2026-07-30): es "pescado con hombros" — no hay objeto de estado (el DOM es la fuente de verdad; cada acción re-lee 11 selects) y **la lista de IDs de filtros está duplicada 5×** (`_BM_FILTROS_IDS` y 4 subsets en `bmSetFiltrosBloqueados`/`bmHayFiltrosActivos`/`resetFiltros`/`_buildFilterParams`) → por eso "cada cambio rompe otro". Etapas: (a) UNA constante `FILTROS=[{id,param,persiste,bloquea}]` única, elimina la duplicación (riesgo bajo, mayor beneficio); (b) objeto de estado `bmEditState` (= Fase 1 de `docs/programa-versiones/`… ver refactor editor barras); (c) partir `barmanager.js` (1040 líneas, 5 trabajos) en render/datos/cobertura. **Regla:** funciones con handler inline en `bar_manager.html` o llamadas desde obras/admin/reclamos (`loadFilters`, `buscar`, `verBarrasCarga`, `onFilterChange`, etc.) NO se renombran sin alias — los renames se silencian (`typeof`) y no dan error. Una etapa = un commit verificable, comportamiento idéntico. | ☐ | YO |
