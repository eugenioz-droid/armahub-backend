...
| 5M.5 | **Fase 5 — Aviso en re-import.** En el preview del import (`/import/armadetailer/preview`), se cuentan las barras con `editado_por` en los (eje,piso,ciclo) que el CSV pisaría: `editadas_replace` por piso + `total_editadas_a_reemplazar` + `editadas_a_reemplazar` (detalle quién/cuándo/marca). Front: warning rojo destacado por bloque ("N editadas a mano se reescribirán… omite si no quieres perderlas"), columna ✏️ Editadas por piso, y contador en el resumen del bloque. | ☑ | YO |
| 5M.6 | **Fase 6 — Color en matrices.** `/dashboard/sectores` expone `editadas` por celda (COUNT FILTER editado_por). En la matriz de exportación (`exportacion.js`), las celdas con edición-plataforma llevan **borde marrón izquierdo + badge ✏️** (marrón #8d6e63, mismo que "editado" en Bar Manager), ORTOGONAL al rosado de "modificado por re-import". Leyenda añadida en exportacion.html (exportado/re-import/editado-a-mano). | ☑ | YO |
