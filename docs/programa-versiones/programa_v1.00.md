...
| FIX 5M.11 | **Filtro Eje/Losa NO aplica al elegir + reaparece sugerencia de contraseña de Chrome.** **CAUSA RAÍZ (auditada 2026-07-30, 6 subagentes):** eran DOS bugs distintos, ninguno del tipo de campo. (1) *No filtra:* la vista agrupada usa `GET /barras/elementos`, cuyo endpoint **no acepta el parámetro `eje`** (`barras.py:225`) → FastAPI descarta el `eje` en silencio y nunca aplica `AND eje=%s`. (2) *Pide clave:* hay un `<input type="password">` real de crear-usuario (`admin.html:37`) en el MISMO DOM de `/ui` (app de una sola página) → Chrome marca TODOS los text inputs como credenciales; el blindaje por-campo no puede ganarle. **Solución FINAL (validada por usuario 2026-07-30):** eje = **input de texto BUSCABLE** (input+datalist), clonado EXACTO del buscador de obra que sí funciona — porque una obra puede tener ~140 ejes y un `<select>` es inservible. Se probó `<select>` en el camino y se descartó por eso. Funciona porque se arregló la RAÍZ: FIX 5M.12 (backend filtra eje) + FIX 5M.13 (password fuera del DOM → ya no pide clave). Patrón estandarizado en SPECS §2.4. Commits `a8c6307`, `5599114`, `20ec3b5`. | ☑ | YO |
| FIX 5M.12 | **Backend: `GET /barras/elementos` acepta y aplica el filtro `eje`** (`barras.py`, `eje` en la firma + `AND TRIM(eje)=TRIM(%s)`). Aplicado también en `/barras` (vista plana) para que ambos endpoints filtren idéntico. El TRIM blinda el bug latente de espacios del CSV (`24(A-E)` vs `24 (A-E)`). | ☑ | YO |
| FIX 5M.13 | **Quitar el `<input type="password">` persistente del DOM de `/ui`** (`admin.html`, crear-usuario): arranca `type="text"` (+`-webkit-text-security:disc` para verse con puntos) y se promueve a `password` por JS solo mientras el form "Nuevo usuario" está visible (`admin/index.js` `toggleNuevoUsuario`). Sin password en reposo, Chrome deja de tratar los filtros de texto como credenciales. Transversal. | ☑ | YO |
| 5M.14 | **Refactor Bar Manager (anti-fragilidad) — EN CURSO, por etapas.** Diagnóstico (auditoría 2026-07-30): es "pescado con hombros" — no hay objeto de estado (el DOM es la fuente de verdad; cada acción re-lee 11 selects) y la lista de IDs de filtros estaba duplicada ~9×. **Etapa (a) HECHA:** UNA constante `BM_FILTROS` (`filtros.js`, `window.BM_FILTROS`) con flags (grupo/param/persiste/limpiaProy/snapshot/esFoco/sanea/activaVistaPlana); 8 listas antes duplicadas ahora se DERIVAN de ella (verificado 1:1 por subagente, comportamiento idéntico); de paso se corrigió el bug latente de `filtroOrigen` (no se limpiaba al cambiar de obra). Pendiente: **(b)** objeto de estado `bmEditState`; **(c)** partir `barmanager.js` (~1040 líneas) en render/datos/cobertura; opcional: unificar `_buildFilterParams` facetas con flag `sanea`, limpiar el `q` muerto. **Regla:** funciones con handler inline en `bar_manager.html` o llamadas desde obras/admin/reclamos NO se renombran sin alias (los renames se silencian por `typeof`). Una etapa = un commit verificable, comportamiento idéntico. | 🔄 | YO |
...
| 5N.20 | **Cableado sub-paso 1 — Contexto real.** Comboboxes Obra (BuscadorObra) · Ciclo (Combobox textoLibre, nuevo) · Eje (Combobox textoLibre) con `/filters`; al elegir obra cargar ejes/ciclos/figuras. Subtabs de tipología ↔ `marca`. Estado en JS. Componente BuscadorObra extraído (usado por Bar Manager Y creador). | ☑ | YO |
| 5N.21 | **Cableado sub-paso 2 — Grilla editable.** Shape backend (dim_a..i/marca), celdas φ(select)/cant/mult/dims/áng/R inputs, figura(datalist catálogo); re-render GRANULAR (mutación directa; figura re-pinta su fila; estructura re-render completo); dims dinámicas por `parciales/angulos/radio`. Handlers +barra/+barras M/copiar/duplicar/quitar. FIX: en TODOS la barra nace SIN tipología (no MH forzado). | ☑ | YO |
...
| 5N.24 | **Cableado sub-paso 5 — Guardado real + lote + "Crear Eje".** 💾 `ac2Guardar`: crea lote (`POST /lotes`) si no existe → `POST /lotes/{id}/barras` (solo barras completas y válidas; contexto sector/ciclo/eje estampado; el check "revisada" VIAJA con cada barra — nuevo campo `revisada` en `BarraManual`+INSERT+trazabilidad). Las guardadas quedan en la grilla marcadas `_guardada` (para revisar/terminar), no se re-insertan. 🏁 `ac2ToggleTerminado` real: `POST /lotes/{id}/terminar` (backend exige todas revisadas + lote no vacío, 5N.19; muestra 409). ✕ descarta lo no guardado. ＋Crear Eje fija el eje de la tanda. Al cambiar de obra se resetea el form (lote guardado queda en BD). **Pendiente:** listado de lotes por obra (backend no tiene GET /lotes?proyecto), retomar lote en avance. | ☑ | YO |
| 5N.24b | **FILA 2 — Sector + Estructura + tipologías dinámicas + bloqueo (se saltó en 5N.24).** Sector (chips fijos FUND/ELEV/VCIELO/LCIELO) y Estructura (MURO/LOSA/VIGA/COLUMNA/FUNDACION/GENERAL) seleccionables → guardan `AC2.sector`/`AC2.estructura`. **Tipologías DINÁMICAS por estructura** (desde `/tipologias`, no hardcodeadas a MURO). Sector+Estructura se BLOQUEAN al elegir; desbloqueo solo si el lote no tiene barras. Crear barras solo con sector+estructura elegidos. | ☐ | YO |
...
| 5N.26 | **Panel "⚙ Configuración de obra" (modal en el creador, botón junto a Crear Eje). DISEÑO CERRADO 2026-07-31.** Config guardada POR OBRA, persiste entre lotes. Secciones: **(1) Pisos** — lista = DERIVADOS de barras (lo que ya existe) + PLANTILLA configurable (default S2,S1,P1..P10,SM); el usuario solo AGREGA los que falten; NO se puede borrar un piso con barras (para eso se limpia la data). Alimenta el desplegable de piso de la grilla. **(2) Ciclos** — igual que pisos. **(3) Ejes** — derivados + AGREGAR nuevos; NO se editan aquí (editar eje = Bar Manager, lo más cercano a la fuente; se descartó homologación automática por riesgo de error). **(4) Factor de peso** — % por obra (columna `proyectos.factor_peso` YA existe; falta endpoint GET/PATCH). **(5) Valores por defecto** — factor extremos (10×φ) y largo intermedio (100), editables por obra. MODELO: la config es "plantilla de opciones", NO compite con lo derivado de barras (una sola fuente de verdad = barras; la plantilla solo sugiere opciones para elegir al crear). Evita el problema que hizo revertir `obra_ejes`/`obra_losas` en la migración 49. **Precisiones (2026-07-31):** el piso de la grilla pasa a `<select>` combinado (derivados+plantilla). **＋barra** → nace en el piso MÁS BAJO configurado, ya poblado. **＋barras M** → una línea por piso seleccionado, cada una con SU piso poblado. Default de pisos (S2,S1,P1..P10,SM) SOLO si la obra está limpia; si ya tiene pisos, toma los existentes y el usuario agrega. | ☐ | YO |
| 5N.29 | **Restricciones en el creador (FUTURO — diseño más adelante, solo andamiaje ahora).** Sección "Restricciones" en el panel de config con selector de qué regla aplica: **Normativa / Fabricación / Particulares Barras**. Las reglas generan BLOQUEOS en la grilla según lo elegido (ej. límites de largo/φ por normativa o por máquina). Las "Particulares Barras" se DEFINEN en el Catálogo (tab tipologías). **Override:** casos específicos donde producción autoriza una medida especial. Ver SPECS §4.7. No bloqueante; solo dejar el lugar en el menú por ahora. | ☐ | TÚ+YO |
...
| 5N.30 | **BUG edición masiva inconsistente (2026-07-31).** Al editar φ con varias filas marcadas a veces cambiaba solo esa fila. Causa: la selección vivía en los checkboxes del DOM y el re-render de una fila borraba el `checked`. Fix de diseño: la selección pasa a estado JS (`AC2.seleccion={id:true}`); la fila se pinta desde ese estado, no al revés. | ☑ | YO |
| 5N.31 | **Orden/agrupación de la grilla (2026-07-31).** Toggle de 3 valores **Creación / Piso / Tipología**, visible en subtabs Y en TODOS (`ac2SetOrden`, `AC2.orden`). Creación = orden de inserción sin agrupar + separador visual sutil entre filas de distinto piso. Piso/Tipología = agrupado con header por grupo. Al agrupar por PISO, cada header trae flechas ▲▼ para reordenar el grupo en la grilla (`ac2MoverGrupo`, orden manual en `_ac2PisosOrden`). El `data-grp` de cada fila refleja el campo de agrupación vigente para que el maestro del grupo marque a sus hijos. | ☑ | YO |
| 5N.32 | **Acciones masivas — 2º nivel de duplicado (2026-07-31).** Barra de acciones masivas reorganizada en dos niveles. Nivel 2: **duplicar +piso** (clona las marcadas y las sube un piso: P1→P2), **duplicar -piso** (las baja), **duplicar seleccionados** (mismo piso). `ac2DuplicarPiso(dir)` mueve el piso una posición en `_ac2Pisos` (índice 0 = más bajo). Warning si la obra no tiene pisos configurados o si las barras ya están en el tope/piso más bajo. **Ordenado:** las copias se insertan JUNTAS tras la última original (no intercaladas), en orden, y quedan SELECCIONADAS para encadenar +piso repetido. | ☑ | YO |
| 5N.33 | **FUND siempre al fondo del orden de pisos (2026-07-31).** `_piso_order` (barras.py + obra_config.py, fuente única): FUND/FUNDACIÓN → −1000000 (bajo todo), SM/PM → 9999 (sobre todo), subterráneos S2\<S1, luego P1..Pn. Confirmado con el usuario: SM siempre arriba, FUND siempre abajo; el resto lo define el orden del usuario (texto libre futuro). | ☑ | YO |
| 5N.34 | **Límite a lados chicos en el render de figura (2026-07-31).** Si un lado es enorme y otro diminuto, el motor escalaba todo al viewport y el chico se perdía. Ahora ningún lado se dibuja menor a `AC2_MIN_LADO_REL` (28%) del lado más grande. Se sacrifica algo de proporción realista (aceptado por el usuario) a cambio de que todos los lados sean visibles. | ☑ | YO |
| 5N.35 | **Check macro "Todas" + duplicados pre-seleccionados (2026-07-31).** (a) Check MACRO en la barra de acciones masivas que marca/desmarca TODAS las barras visibles sin importar el grupo/piso (`ac2SelTodo`), con estado todas/algunas/ninguna. (b) `ac2SelGrupo` compara contra el campo de agrupación vigente (piso o marca) → el maestro de grupo también funciona agrupando por piso (antes solo por marca). (c) Los duplicados (`+piso`/`-piso`/`seleccionados`) quedan PRE-SELECCIONADOS para editarlos en masa sin marcar nada. (d) `ac2Render` re-sincroniza contador/maestros/macro desde el estado. | ☑ | YO |
| 5N.36 | **Filtro de obra lista TODAS las obras (2026-08-03).** El buscador de obra (`/filters`, creador + Bar Manager) sacaba la lista de un `SELECT DISTINCT id_proyecto FROM barras` → omitía obras sin barras (no elegibles en el creador). Fuente correcta = tabla `proyectos` (todas las obras autorizadas). El importer siempre registra ahí el proyecto, así que ninguna con barras se pierde y se suman las vacías. | ☑ | YO |
| 5N.37 | **Datalist de figuras vacío tras re-montaje (2026-08-03).** El datalist de figuras solo se poblaba en `loadAgregarCubicacion2`; `_ac2Init` no cargaba figuras, y el guard de `_ac2CargarFiguras` retornaba temprano dejando el datalist re-inyectado vacío (visible en obras sin barras). Fix: separar descarga (una vez, cacheada) de poblar el datalist (`_ac2PoblarDatalistFiguras`, en cada montaje); `_ac2Init` carga figuras+tipologías. | ☑ | YO |
| 5N.38 | **Crear lote explícito + ubicación obligatoria + eliminar lote (2026-08-03).** Rediseño de flujo (fin de "lotes corruptos"). (a) **Crear lote** es un paso EXPLÍCITO y previo (`ac2CrearLote`): exige Obra + **Ciclo** + **Eje** (ambos obligatorios). Sin lote creado, Sector/Estructura y +barra están BLOQUEADOS (`ac2Bloqueado` incluye `!loteId`). (b) Se reconcilia el TEXTO de los comboboxes ciclo/eje al estado antes de crear/guardar (`_ac2LeerContexto`) → se acaba la carrera del blur que perdía lo escrito (causa raíz del lote sin ciclo/eje). (c) Validación dura en **backend** (`agregar_barras` rechaza barras sin sector/ciclo/eje). (d) **Eliminar lote** (`ac2EliminarLote` + `DELETE /lotes/{id}`): única forma de borrar un lote, pide escribir ELIMINAR, aplica INCLUSO a terminados/bloqueados (única acción posible ahí); borra lote + barras manuales + marca sectores modificados. (e) Cabecera dinámica (`ac2ActualizarCabecera`): sin lote → botón Crear lote (pintado vivo si ubicación completa); con lote → Guardar/Terminar/Descartar/Eliminar; terminado → solo Eliminar. Alinea con `diseno_editor_cubicacion.md` §146-171. | ☑ | YO |
| 5N.39 | **Botón Crear lote unificado (2026-08-03).** El botón "＋ Crear Eje" (junto a los campos de contexto) pasa a ser "Crear lote" (`ac2_crearLoteBtn`), se pinta vivo al completar Obra+Ciclo+Eje. Se elimina el botón Crear lote extra de la cabecera y la función muerta `ac2CrearEje`. El pintado es robusto: relee las 3 fuentes del DOM + listener nativo `input` en obra/ciclo/eje. | ☑ | YO |
| 5N.40 | **BANDERA no reconocía revisadas (2026-08-03).** El check "Rev" vivía solo en el front; la BD no se enteraba → terminar veía "N con ticket = N sin revisar". Fix: `ac2ToggleRev` persiste al instante (`/revisar`) si la barra ya está guardada; `ac2ToggleTerminado` guarda avance primero + sincroniza revisada de TODAS las guardadas antes de terminar. `agregar_barras` devuelve los ids de BD y `ver_lote` incluye `id` → el front asocia `_dbid` a cada barra. | ☑ | YO |
| 5N.41 | **Flechas de reorden de piso (2026-08-03).** `ac2MoverGrupo` parte siempre del orden visible actual (swap consistente aunque `_ac2PisosOrden` esté vacío). Las flechas ▲▼ son botones claros con etiqueta "mover piso" en el header del grupo, solo en modo agrupado por piso. | ☑ | YO |
| 5N.42 | **Columna Multiplicador + Cant.T + Sufijo de tipología (2026-08-03).** (a) Columnas **Mult** (editable) y **Cant.T** (=cant×mult, solo lectura) entre Cant y Largo. El peso ya usaba cant×mult (no se toca → sin doble conteo); `cant_total`/`mult` ya viajaban de punta a punta (auditado). (b) **Sufijo de tipología** (`suf_tipo`, migración 093): campo de texto a la derecha de Tipología que se CONCATENA a MARCA **solo al exportar a aSa** (`export.py`); la tipología interna (`marca`) NO cambia → dashboards sin inconsistencias. Persiste en `agregar_barras`, se restaura al retomar, viaja en payload. | ☑ | YO |
| 5N.43 | **Foco de la grilla + navegación por teclado (2026-08-03).** `ac2ReRenderFila` recibe `focoCol` EXPLÍCITO (no deduce de `document.activeElement`, que en select/`input list` ya perdió el foco) → figura/φ dejan de perder el cursor. Selects φ/marca/sufijo ahora navegables (`ac2nav`+`data-col`+`onkeydown`); `ac2NavKey` incluye selects; Enter hace `preventDefault` aunque no haya fila destino. Check de selección separado visualmente de la columna Piso (borde+padding). Botón Acciones masivas AZUL estilo Bar Manager; layout reordenado (acciones a la izquierda, visualización a la derecha). | ☑ | YO |
| 5N.44 | **Botón "＋ Crear obra" en tab Clientes (2026-08-03).** Tras limpiar la plataforma no había forma de crear obras desde la UI. Botón + modal (nombre*, clasificación, empresa, calculista, descripción) en el sub-panel Obras/Tiendas → `POST /proyectos` (endpoint existente). Gate admin/admin_calidad, anti-duplicado por nombre. | ☑ | YO |

| 5N.46 | **Render muestra el VALOR del lado, no la letra (2026-08-03).** El modo escalable (letra→medida) se excluía si la figura tenía CUALQUIER etiqueta, aunque fuera decorativa → una figura simple recreada con etiqueta residual (ej. 101A) mostraba "A" en vez de "10". Ahora solo las figuras etiqueta-manda o con radio caen al dibujo original; al escalar se limpian las etiquetas decorativas. Fallback de "sin geometría" muestra "sin dibujo · CÓDIGO" (no el código crudo). | ☑ | YO |
| 5N.47 | **Orden asc/desc de pisos + guardar/X + correlativo por obra (2026-08-03).** (a) Iconos ▲/▼ en el ENCABEZADO de la columna Piso, visibles en las 3 vistas (`ac2OrdenarPisos`, `_ac2PisoDir`); al presionarlos fuerzan orden por piso. Distinto de las flechas "mover piso" por-grupo. (b) **El disquete NO limpia** (guardar avance seguido sin perder info); **la X limpia** el formulario y vuelve a crear lote (`_ac2ResetTanda`); no borra de la BD las barras guardadas. (c) **Correlativo de lote POR OBRA** (`num_obra`): "Lote #1, #2…" por obra, no el id global. | ☑ | YO |
| 5N.47b | **Correlativo FIJO + lápida al eliminar lote (2026-08-03).** `num_obra` se asigna UNA vez al crear (`MAX(num_obra)+1` por obra, columna en migración 094) y NUNCA se recalcula ni reusa — antes se derivaba de la posición (ROW_NUMBER) → borrar un lote renumeraba los siguientes (bug de referencia). Al **eliminar**, el lote NO se borra: se marca `estado='eliminado'` + snapshot (n_barras/kg/sector/ciclo/eje) + quién/cuándo (LÁPIDA); las barras sí se borran. El histórico muestra la lápida en gris/tachado, sin link, con quién la eliminó. El número queda como hueco (como factura anulada). Backend: crear/eliminar/listar/ver actualizados; front pinta la lápida. | ☑ | YO |
| 5N.45 | **Figura sin `parciales` → "geometría inválida" falsa (2026-08-03).** Causa raíz: el Diseñador podía guardar una figura con `parciales=[]` (modo etiquetas etiquetando solo un ángulo/radio, sin lados) y el seed usa `ON CONFLICT DO NOTHING` → no la corrige. Sin `parciales`, la validación de barras marca la figura inválida (le "sobran" las medidas). Caso real: 101A (1 lado A) quedó con `parciales` vacío → "A sobra". **Fix de RAÍZ (no defensas):** (1) DATO — `reparar_parciales_vacios` corre en el arranque y corrige en BD toda figura con `parciales` vacío + geometría presente (deriva de `geometria.tramos`), idempotente; (2) ORIGEN — en el Diseñador los lados (`parciales`) salen SIEMPRE de los tramos dibujados, las etiquetas solo aportan ángulos/radio (lógica reescrita limpia). Se QUITARON las defensas provisionales de `get_figura`/`ac2DimsDeFigura`. Además: fila inválida resaltada (rojo tenue) + tooltip con cuál barra y qué revisar. | ☑ | YO |

| 5N.48 | **Duplicar despiece + descartar vacío + resalte masiva + varios (2026-08-04).** (a) **Duplicar despiece** (`POST /lotes/{id}/duplicar` + botón en el histórico + popup ciclo/eje): crea un despiece nuevo con toda la data del origen (piso/sector/tipología/figura/dims/cant/mult/sufijo), solo cambia ciclo/eje; funciona también desde lápida (snapshot). (b) **Descartar despiece vacío** (`DELETE /lotes/{id}/vacio`): un despiece creado y nunca guardado (0 barras) se borra físicamente al abandonarlo (X, cambiar obra, retomar/crear otro, terminar) → no ensucia el histórico y libera su `num_obra`. (c) **Resalte azul** de fila seleccionada en edición masiva (creador y Bar Manager). (d) Bug figura→figura de menos lados dejaba slots pegados (ahora `ac2AplicarDefaults` limpia los que la nueva figura no usa). (e) Foco en masiva (Enter ya no lo pierde). (f) Enter avanza a la derecha (como Tab). (g) Sector↔Estructura: al elegir sector, se deshabilitan estructuras no válidas (ELEV→MURO/COLUMNA/GEN, LCIELO→LOSA/GEN, VCIELO→VIGA/GEN, FUND→FUNDACION/GEN). (h) Barra eliminada de un despiece guardado se borra también en BD (endpoint `DELETE /lotes/{id}/barras/{bid}`). (i) Ver despiece eliminado en solo-lectura (snapshot `snap_barras` JSONB, migración 095). | ☑ | YO |
| 5N.49 | **Nomenclatura Items vs Barras físicas (2026-08-04).** Estándar: **items** = nº de entradas/filas (`COUNT`); **barras** = barras físicas (`Σ cant_total` = cant×mult). Aplicado en: Bar Manager (KPIs agrupada/plana + detalle ciclo), creador (rollup), histórico de despieces (columnas Items/Barras), `/stats` (`total_barras_fisicas`), constructoras, calculistas, `get_proyectos`, obras, clientes. `mult` nunca queda vacío (creador vuelve a 1; Bar Manager sincroniza cant/mult al editar cant_total con trazabilidad; migración 096 backfill mult NULL). | ☑ | YO |
| 5N.50 | **KPIs de productividad incluyen cubicación manual (2026-08-04).** El importador ahora estampa `creado_por` en cada barra CSV (antes solo en `imports`); migración 097 backfill de barras CSV viejas desde `imports.usuario`. KPIs migrados de `imports`→`barras` (por `creado_por`, `fecha_carga`): `stats/cubicadores`, `cubicacion-mensual`, `timeline`, `mi-actividad`, `landing/cubicado_semana`. Ahora incluyen CSV + manual; "barras" = Σ cant_total, "cargas" (nº CSV) sigue de `imports`. | ☑ | YO |
| 5N.51 | **Partir limpio al re-entrar + rename a "Despiece" (2026-08-04).** (a) Al (re)entrar al tab (volver de otra sección) el formulario parte LIMPIO (resetea despiece a medias; descarta el vacío; conserva la obra). (b) Rename SOLO de textos VISIBLES `lote/cubicación`→**Despiece** (tab "Agregar Despiece", "Despiece de Cubicación", "Nuevo Despiece", botones/badges/alerts, histórico "Despieces de la obra", "Despiece #N"). El código interno (tabla `lotes`, endpoints `/lotes`, `loteId`) NO se toca — decisión: la capa de datos no cambia por una etiqueta de UI. | ☑ | YO |

### PENDIENTES (documentados, orden acordado con el usuario 2026-08-04)

El usuario definió este ORDEN para retomar (tras 5N.51): **(P5) Bar Manager mejoras → (P3) restricciones creador → (P4) radios geometría real → (P6) multi-catálogo/BVBS**. Además una idea nueva a explorar: **obtener data desde un CAD** (el usuario la explicará al terminar P3).

| P1 | **Volver de otra sección → partir limpio.** ✅ HECHO en 5N.51. | ☑ | YO |
| P2 | **Nombre de "lote" → "Despiece".** ✅ HECHO en 5N.51 (textos visibles). | ☑ | YO |
| P3 | **Restricciones en el creador (5N.29).** Implementación SENCILLA primero, con posibilidad de expandir: bloqueos por normativa/fabricación (límites de largo/φ). Se define y arma tras 5N.51. Ver SPECS §4.7. | ☐ | TÚ+YO |
| P4 | **Radios con propiedades geométricas reales (5N.27, deuda).** Hoy el radio es solo un valor sin semántica → el render no re-escala esas figuras. Darle radio/cuerda/desarrollo reales. Después de P3. | ☐ | TÚ+YO |
| P5 | **Bar Manager — mejoras varias.** Orden por piso (con `_piso_order` lógico: FUND abajo, SM arriba, S2\<S1; desempate sector→eje→ciclo), más filtros de orden (piso/tipología), y otros. Investigación previa hecha (vista agrupada ya ordena por piso simplificado; vista plana no). Congelado hasta que el usuario entre a editar. Va ANTES de P3/P4 según el usuario. | ☐ | TÚ+YO |
| P6 | **Multi-catálogo / conversores BVBS (`docs/programa_multicatalogo.md`).** Visión futura, ya planificado; revisar DESPUÉS de P4. NO implementar aún. | ☐ | TÚ+YO |
| P7 | **Obtener data desde un CAD (NUEVO, a explorar).** El usuario quiere una función que apunte a extraer data de un CAD. La explicará al terminar P3. Solo anotado. | ☐ | TÚ+YO |

**PENDIENTE (datos, no código):** subtab MV (y cualquiera) muestra "letras" en el render cuando la figura elegida NO tiene geometría poblada en el catálogo → cae al fallback que dibuja el código. El render es agnóstico a la tipología; MH y MV comparten figuras. **Acción:** poblar la geometría de esas figuras desde el Diseñador (tab Catálogo).
