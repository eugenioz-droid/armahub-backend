...
| 5N.47 | **Orden asc/desc de pisos + limpiar tras guardar + correlativo por obra (2026-08-03).** (a) Iconos ▲/▼ en el ENCABEZADO de la columna Piso (uno solo) para ordenar TODOS los pisos asc/descendente (`ac2OrdenarPisos`, `_ac2PisoDir`); solo al ordenar por piso. Distinto de las flechas "mover piso" por-grupo. (b) Al guardar (💾), si TODAS las barras quedaron persistidas, se resetea el formulario y se vuelve a "crear lote" (`_ac2ResetTanda`); si quedan a medio llenar, NO se limpia. (c) **Correlativo de lote POR OBRA** (`num_obra`): el usuario ve "Lote #1, #2…" por obra, no el id global (#7). El id SERIAL se mantiene para la API; backend calcula `num_obra` (ROW_NUMBER/COUNT por obra) en crear/ver/listar. | ☑ | YO |
