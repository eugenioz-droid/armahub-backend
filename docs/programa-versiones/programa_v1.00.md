...
| 5N.45 | **Figura sin `parciales` → "geometría inválida" falsa (2026-08-03).** Causa raíz: el Diseñador podía guardar una figura con `parciales=[]` (modo etiquetas sin lados) y el seed usa `ON CONFLICT DO NOTHING` → no la corrige. Sin `parciales`, la validación de barras marca la figura inválida (le "sobran" las medidas). Caso real: 101A (1 lado A) quedó con `parciales` vacío → "A sobra". **Triple defensa:** (1) Diseñador nunca guarda `parciales` vacío si dibujó lados (deriva de tramos); (2) `get_figura` backend deriva de `geometria.tramos` si `parciales` vacío; (3) `ac2DimsDeFigura` front idem → las figuras ya guardadas mal validan bien sin re-crearlas. Además: la fila inválida se resalta (fondo rojo tenue) y el contador "⚠ N inválida" tiene tooltip con cuál barra y qué revisar. | ☑ | YO |
